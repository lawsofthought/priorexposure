...
M.1 <- jags.model("model.1.jags", n.chain=3)
update(M.1, n.iter)
...
M.2 <- jags.model("model.2.jags", n.chain=3)
update(M.2, n.iter)
...
M.3 <- jags.model("model.3.jags", n.chain=3)
update(M.3, n.iter)
...
pitch.mcmc.3 <- MCMCglmm(pitch ~ base + attract, 
                         random= ~ Participant, 
                         prior = list(
                           B = list(mu = c(0, 0, 0), V = diag(3) * 1e3)
                         ),
                         nitt=n.iter, 
                         data=pitch.df)
...
M.4 <- jags.model("model.4.jags", n.chain=3)
update(M.4, n.iter)
...
M.5 <- jags.model("model.5.jags", n.chain=3)
update(M.5, n.iter)
...
M.6 <- jags.model("model.6.jags", n.chain=3)
update(M.6, n.iter)
...
M.7 <- jags.model("model.7.jags", n.chain=3)
update(M.7, n.iter)
